        ...
        conflictAbles.forEach {
            ...
            fun checkWakeConflictForAircraftInSector(sector: GdxArray<WakeZone>): Boolean {
                for (i in 0 until sector.size) sector[i]?.apply {
                    ...
                    // TODO Very bad temporary workaround to inhibit wake turbulence warnings on LDA PRM 28R - find better generalizable fix ASAP
                    val visAppName = it[VisualCaptured.mapper]?.parentApp?.get(ApproachInfo.mapper)?.approachName
                    val positiveOffset = if (visAppName == "LDA PRM 28R") 0 else 90
                    // Check that aircraft altitude is between 90 feet above and 950 feet below the wake altitude
                    if (alt.altitudeFt > wakeAlt.altitudeFt + positiveOffset || alt.altitudeFt < wakeAlt.altitudeFt - 950) return@apply
