...
class PublicServerV2(
    ...
    override fun beforeStart(): Boolean {
        ...
        val ciphertext = encryptor.encryptWithIV(iv, com.bombbird.terminalcontrol2.networking.relaygateway.RelayNonce(roomCreation.authResponse.nonce))?.ciphertext
        if (ciphertext == null) {
    ...
    override fun requestGameCreation() {
        relayConn.sendTCP(
            ProtoMessages.newGameRequest(roomId.toInt(), gameServer.maxPlayersAllowed.toInt(), mapName, myUuid.toString())
        )
    }

    override fun decodeRelayMessageObject(data: ByteArray, sendingUUID: UUID, clientToRelayRtt: Int) {
